...
impl<T> LinkedList<T> {
	...
	{    let mut merged_list = LinkedList::new();
        ...
        while curr_a.is_some() && curr_b.is_some() {
            let val_a = unsafe { curr_a.unwrap().as_ref().val.clone() };
            let val_b = unsafe { curr_b.unwrap().as_ref().val.clone() };
